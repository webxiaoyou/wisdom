	...
	<view>
		<u-popup :show="loginPopupShow" mode="bottom" :round="10" @close="closeLogin" zIndex="999998">
			<view class="f__login">
				...
				<view class="text">会员用户登录后享受更好的服务体验</view>
				<view class="loginButton" v-if="!isPhoneLogin">
					...
					<!-- <button class="button marginT" open-type="getPhoneNumber" @getphonenumber="decryptPhoneNumber" :style="{background:PrimaryColor}">微信手机号登录</button> -->
					<!-- 此功能需微信认证 -->
					<!-- #endif -->
					<!-- #ifdef MP-ALIPAY -->
					<button class="button" open-type="getAuthorize" scope="userInfo" @getAuthorize="onAuthorization"
						:style="{background:PrimaryColor}">支付宝授权登录</button>
					<button class="button marginT" open-type="getAuthorize" scope="phoneNumber"
						@getAuthorize="decryptPhoneNumber" @error="onAuthError"
						:style="{background:PrimaryColor}">支付宝手机号登录</button><!-- 此功能需申请 -->
					<!-- #endif -->
	...
	export default {
		...
		methods: {
			...
			getLoginByUnion() {
				...
				uni.login({
					success(res) {
						console.log(res, "获取code",courtConfig.publicAppId)
						that.code = res.code
						let httpData = {
