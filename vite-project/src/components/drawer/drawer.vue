    ...
    <a-drawer width="60%" placement="right" :closable="false" :open="opens" @close="onClose">
      <a-descriptions bordered title="详情" :column="{ xxl: 2, xl: 2, lg: 2, md: 1, sm: 1, xs: 1 }">
        <a-descriptions-item v-for="column in columns" :label="column.title" :key="column.dataIndex">
          <template
            v-if="column.dataIndex === 'avatar' || column.dataIndex === 'imageUrl' || column.dataIndex === 'coverUrl'">
            <a-image :width="80" :src="formData[column.dataIndex]"></a-image>
          </template>
          <template v-else-if="column.dataIndex === 'imageUrls'">
            <template v-if="formData[column.dataIndex]">
              <a-image :width="80" :src="item" v-for="(item, index) in formData[column.dataIndex].split(',')"
                :key="index"></a-image>
            </template>
          </template>
          <template v-else-if="column.dataIndex === 'authenticated'">
            <a-tag
              :color="formData[column.dataIndex] == 0 ? 'success' : formData[column.dataIndex] == 1 ? 'error' : 'warning'">
              {{ getDictLabel(props.distList, 'puser_authenticated', formData[column.dataIndex]) }}</a-tag>
          </template>
          <template v-else-if="column.dataIndex === 'authenticationStatus'">
            <a-tag
              :color="formData[column.dataIndex] == 0 ? 'warning' : formData[column.dataIndex] == 1 ? 'success' : 'error'">
              {{ getDictLabel(props.distList, 'audit_status', formData[column.dataIndex]) }}</a-tag>
          </template>
          <template v-else-if="column.dataIndex === 'content'">
            <!-- <div style="width: 100%; max-width: 800px;" v-html="formData[column.dataIndex]"></div> -->
            <editor v-model="formData[column.dataIndex]" :disabled="true" :readonly="true" />
          </template>
