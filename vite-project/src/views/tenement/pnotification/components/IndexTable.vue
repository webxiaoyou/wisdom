<template>
  <a-table :row-selection="rowSelection" :columns="columns" :scroll="{ x: 1500 , y: height - 450}" class="custom-table"
    :data-source="data" rowKey="notificationId" :pagination="false">
    <template #bodyCell="{ column, text, record,index }">
      <template v-if="column.dataIndex === 'notificationType'">
        <a-select placeholder="岗位" v-model:value="record.notificationType" :open="false" :showArrow="false" :bordered="false">
          <a-select-option :value="item.dictValue" v-for="item in distList?.sys_notice_type" :key="item.dictValue">{{
      ...
      <template v-if="column.dataIndex === 'propertyId'">
        <a-select placeholder="岗位" v-model:value="record.propertyId" :open="false" :showArrow="false" :bordered="false">
          <a-select-option :value="item.value" v-for="item in distList?.pproperty_dict" :key="item.dictValue">{{
      ...
      <template v-if="column.dataIndex === 'operation' && record.roleName != '超级管理员'">
        <a-button type="link" @click="onQuery(record.notificationId, 'up')" v-hasPermi="['tenement:pnotification:edit']">
          <template #icon>
            <Icon icon="uil:edit" style="vertical-align: middle;" />
          </template>
          编辑
        </a-button>
        <a-popconfirm title="你确定要删除该条数据吗" ok-text="确定" cancel-text="取消" @confirm="onQuery(record.notificationId, 'del')" >
          <a-button type="link" v-hasPermi="['tenement:pnotification:remove']">
            <template #icon>
