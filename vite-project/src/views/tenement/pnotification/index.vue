<template>
  <div class="animate__animated animate__fadeInDown">
    ...
    <a-space class="animate__animated animate__fadeInDown">
      <a-button type="primary" @click="onAdd('add')" style="background-color: #40ce8c;"
        class="flex-row flex-align-center m-b3" v-hasPermi="['tenement:pnotification:add']">
        <template #icon>
      ...
      <a-button type="primary" @click="onAdd('up')" style="background-color: #ffb700;" class="flex-row flex-align-center m-b3"
        :disabled="single" v-hasPermi="['tenement:pnotification:edit']">
        <template #icon>
          <Icon icon="uil:edit" />
        </template>
        编辑
      ...
      <a-button type="primary" danger class="flex-row flex-align-center m-b3" :disabled="multiple"
        v-hasPermi="['tenement:pnotification:remove']" @click="onDel">
        <template #icon>
  ...
  <div class="content animate__animated animate__fadeIn">
    <pindex_table :distList="distList" :data="listData?.rows" :columns="columns" v-model="selectedRows" @query="onQuery"
  ...
  <a-modal v-model:open="opendel" @cancel="AddUpClose" title="你确定要删除以下数据吗？" @ok="onDels(selectedRows)">
    <p v-for="id in selectedRows" :key="id">
      {{ getUserDataById(id).title }}
    </p>
  </a-modal>  
</template>

<script setup lang="ts" name="pnotification">
...
import pindex_table from './components/IndexTable.vue';
import PIndexAddUp from '@/views/tenement/pnotification/components/pindexAddUp.vue';
...
let { distList } = useUserDictData(['sys_notice_type','target_type']);  //字典数据
//表格内容
...
// 请求数据
let formState: UnwrapRef<PNotification> = reactive({
  status: null,
  createTime: null,
  updateTime: null,
  pageNum: 1,
...
let DetailData = ref<PNotification>({}); //详情数据
...
watch(
  ...
  (newValue, oldValue) => {
    // 在 distList 变化时执行的逻辑
    searchColumns.value[2].options = distList.value.sys_notice_type
    FormColumns.value[3].options = distList.value.sys_notice_type
  }
);
// 查询小区字典详情
...
const list = async () => {
  ...
  const { data } = await API.tmt_pnotice_list(requestData)
  listData.value = data
}
...
const onDels = async (id: Number[]) => {
  if (id.length >= 1) {
    const datas: any = await API.tmt_pnotice_del(
      id
    )
...
const onDetail = async (id: number) => {
  const datas: any = await API.tmt_pnotice_id(id);
  const { code, data, msg } = datas;
  if (code === 200) {
...
const callSubmit = async (formState: PNotification, name: string) => {
  console.log(name)
  if (name === '新增') {
    onAddUp(formState)
  } else {
...
//编辑
const onUpdate = async (formStates: PNotification) => {
  const datas: any = await API.tmt_pnotice_up(formStates);
  const { code, msg } = datas
  if (code === 200) {
    message.success(msg)
    list()
...
// 新增操作
const onAddUp = async (callformState: PNotification) => {
  const datas: any = await API.tmt_pnotice_sa(callformState)
  const { code, msg } = datas
  if (code === 200) {
    message.success(msg)
    list()
...
const getUserDataById = (id) => {
  return listData.value.rows.find(user => user.notificationId === id) || null;
}
