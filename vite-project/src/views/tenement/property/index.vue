<template>
  ...
  <a-space class="animate__animated animate__fadeInDown">
    <a-button type="primary" @click="onAdd('add')" style="background-color: #40ce8c;"
      class="flex-row flex-align-center m-b3" v-hasPermi="['system:pproperty:add']">
      <template #icon>
    ...
    <a-button type="primary" @click="onAdd('up')" style="background-color: #ffb700;"
      class="flex-row flex-align-center m-b3" :disabled="single" v-hasPermi="['system:pproperty:edit']">
      <template #icon>
        <Icon icon="uil:edit" />
      </template>
      编辑
    ...
    <a-button type="primary" danger class="flex-row flex-align-center m-b3" :disabled="multiple"
      v-hasPermi="['system:pproperty:remove']" @click="onDel">
      <template #icon>
...
import dayjs from 'dayjs';
import { convertToNumber, calculatePageCount, phoneCheck } from '@/hooks/publicFunction';
...
// 请求数据
let formState: UnwrapRef<PProperty> = reactive({
  status: null,
  createTime: null,
  updateTime: null,
  pageNum: 1,
...
const FormColumns = ref<Array<FormColumns>>([
  ...
  },
  {
    ...
    dataType: 'input',
    name: 'address',
    rules: [{ required: true, message: '请输入小区地址', trigger: 'change' },]
  },
  {
    title: '经理姓名',
    dataType: 'input',
  ...
  },
  {
    ...
    dataType: 'inputNumber',
    name: 'phone',
    rules: [{ required: true, message: '请输入正确的电话', trigger: 'change' },
...
let DetailData = ref<PProperty>({}); //详情数据
...
//   (newValue, oldValue) => {
//     // 在 distList 变化时执行的逻辑
//     searchColumns.value[3].options = distList.value.sys_normal_disable
...
const list = async () => {
  ...
  const { data } = await API.sys_pproperty_list(requestData)
  listData.value = data
}
...
const onDels = async (id: Number[]) => {
  if (id.length >= 1) {
    const datas: any = await API.sys_pproperty_del(
      id
    )
...
const onDetail = async (id: number) => {
  const datas: any = await API.sys_pproperty_id(id);
...
const callSubmit = async (formState: PProperty, name: string) => {
  console.log(name)
  if (name === '新增') {
    onAddUp(formState)
  } else {
...
//编辑
const onUpdate = async (formStates: PProperty) => {
  const datas: any = await API.sys_pproperty_up(formStates);
  const { code, msg } = datas
  if (code === 200) {
    message.success(msg)
    list()
...
// 新增操作
const onAddUp = async (callformState: PProperty) => {
  const datas: any = await API.sys_pproperty_sa(callformState)
  const { code, msg } = datas
  if (code === 200) {
    message.success(msg)
    list()
...
const getUserDataById = (id) => {
  return listData.value.rows.find(user => user.propertyId === id) || null;
}
