<template>
  <a-table :row-selection="rowSelection" :columns="columns" :scroll="{ x: 1500, y: height - 450 }" class="custom-table"
    :data-source="data" rowKey="buildingId" :pagination="false">
    <template #bodyCell="{ column, text, record, index }">
      <template v-if="column.dataIndex === 'buildingNumber'">
        <router-link
          :to="{ name: 'floor-data', params: { buildingId: record.buildingId, buildingNumber: record.buildingNumber } }"
          class="link-type">
          <span>{{ text }}</span>
        </router-link>
      </template>
      ...
      <template v-if="column.dataIndex === 'operation' && record.roleName != '超级管理员'">
        <a-button type="link" @click="onQuery(record.buildingId, 'up')" v-hasPermi="['tenement:pbuilding:edit']">
          <template #icon>
            <Icon icon="uil:edit" style="vertical-align: middle;" />
          </template>
          编辑
        </a-button>
        <a-popconfirm title="你确定要删除该条数据吗" ok-text="确定" cancel-text="取消" @confirm="onQuery(record.buildingId, 'del')">
          <a-button type="link" v-hasPermi="['tenement:pbuilding:remove']">
            <template #icon>
