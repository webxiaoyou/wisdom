<template>
  ...
  <a-modal v-model:open="opendel" @cancel="AddUpClose" title="你确定要删除以下数据吗？" @ok="onDels(selectedRows)">
    <p v-for="id in selectedRows" :key="id">
      {{ getUserDataById(id).appid }}
    </p>
  </a-modal>
</template>

<script setup lang="ts" name="pconfig/index">
...
// 请求数据
let formState: UnwrapRef<PConfig> = reactive({
  status: null,
  createTime: null,
  updateTime: null,
  pageNum: 1,
...
let DetailData = ref<PConfig>({}); //详情数据
...
const list = async () => {
  ...
  const { data } = await API.sys_user_config_list(requestData)
  listData.value = data
}
...
const onDels = async (id: Number[]) => {
  if (id.length >= 1) {
    const datas: any = await API.sys_user_config_del(
      id
    )
...
const onDetail = async (id: number) => {
  const datas: any = await API.sys_user_config_id(id);
...
const callSubmit = async (formState: PConfig, name: string) => {
  console.log(name)
  if (name === '新增') {
    onAddUp(formState)
  } else {
...
//编辑
const onUpdate = async (formStates: PConfig) => {
  const datas: any = await API.sys_user_config_up(formStates);
  const { code, msg } = datas
  if (code === 200) {
    message.success(msg)
    list()
...
// 新增操作
const onAddUp = async (callformState: PConfig) => {
  const datas: any = await API.sys_user_config_sa(callformState)
  const { code, msg } = datas
  if (code === 200) {
    message.success(msg)
    list()
