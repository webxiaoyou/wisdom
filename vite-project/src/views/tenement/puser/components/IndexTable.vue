<template>
  <a-table :row-selection="rowSelection" :columns="columns" :scroll="{ x: 1500, y: height - 450 }" class="custom-table"
    :data-source="data" rowKey="userId" :pagination="false">
    <template #bodyCell="{ column, text, record, index }">
      <template v-if="column.dataIndex === 'userId'">
        <a @click="onQuery(record.userId)">{{ text }}</a>
      </template>
      ...
      <template v-if="column.dataIndex === 'authenticated'"> 
        <a-tag :color="text == 0 ? 'success' : text == 1 ?'error':'warning'">{{ getDictLabel(distList, 'puser_authenticated', text) }}</a-tag>
      </template>
      ...
      <template v-if="column.dataIndex === 'operation' && record.roleName != '超级管理员'">
        <a-button type="link" @click="onQuery(record.userId, 'up')" v-hasPermi="['system:pconfig:edit']">
          <template #icon>
            <Icon icon="uil:edit" style="vertical-align: middle;" />
          </template>
          编辑
        </a-button>
        <a-popconfirm title="你确定要删除该条数据吗" ok-text="确定" cancel-text="取消" @confirm="onQuery(record.userId, 'del')">
          <a-button type="link" v-hasPermi="['system:pconfig:remove']">
            <template #icon>
