...
// 请求数据
let formState: UnwrapRef<SysPost> = reactive({
  postName: '',   //名称
  postCode: '', // 权限字符
  status: null,
  createTime: null,
...
let DetailData = ref<SysPost>({}); //详情数据
...
const list = async () => {
  ...
  const { data } = await API.sys_post_list(requestData)
  listData.value = data
}
...
const onDels = async (id: Number[]) => {
  if (id.length >= 1) {
    const datas: any = await API.sys_post_del(
      id
    )
...
const onDetail = async (id: number) => {
  const datas: any = await API.sys_post_id(id);
...
const callSubmit = async (formState: SysPost, name: string) => {
  console.log(name)
  if (name === '新增') {
    onAddUp(formState)
  } else {
...
//编辑
const onUpdate = async (formStates: SysPost) => {
  const datas: any = await API.sys_post_up(formStates);
  const { code, msg } = datas
  if (code === 200) {
    message.success(msg)
    list()
...
// 新增操作
const onAddUp = async (callformState: SysPost) => {
  const datas: any = await API.sys_post_sa(callformState)
  const { code, msg } = datas
  if (code === 200) {
    message.success(msg)
    list()
